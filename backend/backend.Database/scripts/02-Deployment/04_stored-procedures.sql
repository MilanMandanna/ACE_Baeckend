...
-- Sample EXEC [dbo].[SP_Admin_GetAdminItemsAndDownloadDetails] 102, 'page', 'populations'
-- =============================================
...
-- Sample EXEC [dbo].[SP_Aircraft_GetProducts] 'aircraft id'
-- =============================================
...
BEGIN
	...
	BEGIN
		...
		print @updateFontKey
		UPDATE tblFont
		SET   Size = @tbFontSize, Color = @tbFontColor, ShadowColor = @tbFontShadowColor, FontFaceId = @tbFontFontFaceId, FontStyle = @tbFontFontStyle
...
-- Create date: 06/24/2022
-- Description:	Import Fonts from Asxiinfo.sqlite
-- Sample EXEC [dbo].[SP_AsxiInfoImport_InfoSpelling] 1
-- =============================================
...
BEGIN
 ...
 WHERE CAST(TBIS.InfoId as varchar)+CAST(TBIS.LangID as varchar) NOT IN (SELECT CAST(TBLIS.InfoId as varchar)+CAST(TBLIS.LanguageId as varchar)
 FROM config_tblInfoSpelling(@configid) TBLIS)

  --For update Records
 INSERT INTO @tempUpdateInfoSpelling(InfoId,LangID,InfoItem)
 SELECT TBIS.InfoId,TBIS.LangID, TBIS.InfoItem FROM @tempNewInfoSpellingWithIDs TBIS
	WHERE CAST(TBIS.InfoId as varchar)+CAST(TBIS.LangID as varchar) 
		IN (SELECT CAST(TBLIS.InfoId as varchar)+CAST(TBLIS.LanguageId as varchar)
...
BEGIN
	IF (@type = 'all')
	BEGIN
		SELECT DISTINCT inset.ASXiInsetID,inset.InsetName,ISNULL(inset.IsHf, 0) AS IsHF,ISNULL(inset.IsUHf, 0) AS IsUHf FROM dbo.config_tblASXiInset(@configurationId) as inset 
	END	
	ELSE IF(@type = 'hf')
	BEGIN
...
BEGIN
	...
	BEGIN
		UPDATE inset
		SET inset.IsHf = @selected
		FROM dbo.config_tblASXiInset(@configurationId) as inset
		WHERE inset.ASXiInsetID = @ASXiInsetID
	END	
	ELSE IF(@type = 'uhf')
	BEGIN
		UPDATE inset
		SET inset.IsUHf = @selected 
		FROM dbo.config_tblASXiInset(@configurationId) as inset
		WHERE inset.ASXiInsetID = @ASXiInsetID
	END
END
...
BEGIN
    ...
    BEGIN
        ...
        LEFT OUTER JOIN tblAircraftConfigurationMapping on tblAircraftConfigurationMapping.ConfigurationDefinitionID = tblConfigurations.ConfigurationID)
        LEFT OUTER JOIN Aircraft ON Aircraft.Id = tblAircraftConfigurationMapping.AircraftID)

        WHERE 
        dbo.tblTasks.StartedByUserID = @userId  
...
BEGIN  
 ...
 ELSE IF (@type = 'aircraft')  
  BEGIN  
    ...
    SELECT CC.Name, CC.[ConfigurationComponentID] FROM dbo.config_tblConfigurationComponents(@configurationId) as CC
	WHERE CC.Name IN (SELECT * FROM STRING_SPLIT(@inputstring, ',')) 
  END  
...
BEGIN
    ...
    inner join UserRoleClaims on UserRoleClaims.roleid = UserRoleAssignments.roleid 
    inner join UserClaims on UserClaims.id = UserRoleClaims.claimid 
    inner join aircraft on aircraft.id = UserRoleClaims.aircraftid 
    ...
    inner join operator on aircraft.operatorid = @operatorId 
    where 
    UserClaims.name in ('Manage Aircraft', 'Administer Aircraft') 
    and 
...
BEGIN
	SELECT DISTINCT * FROM
            ...
            where 
            UserClaims.name in ('ManagePlatformConfiguration', 'ManageProductConfiguration', 'Manage Global Configuration') and tblFeatureSet.name like '%editable%'
...
-- Create date: 08/18/2022
-- Description:	Get the default part number
-- Sample EXEC [dbo].[SP_ConfigurationDefinition_DefaultPartNumber] 5036
-- =============================================
...
BEGIN
    ...
    INSERT INTO @partnumbertable SELECT tp.PartNumberID, tp.Name, tp.PartNumberCollectionID, tp.Description, tc.Value  FROM tblConfigurationDefinitionPartNumber tc 
    INNER JOIN tblPartNumber tp ON  tc.PartNumberID =tp.PartNumberID where tc.ConfigurationDefinitionID = @ConfigurationDefinitionId

	INSERT INTO @partnumbertable SELECT pa.PartNumberID, pa.Name ,pa.PartNumberCollectionID,pa.Description,pa.DefaultPartNumber from tblPartNumber pa
	...
	WHERE tc.ConfigurationDefinitionID = @ConfigurationDefinitionId)



	SELECT * FROM @partnumbertable ORDER BY PartNumberID ASC
END
GO
GO
...
-- Sample EXEC [dbo].[SP_ConfigurationDefinition_UpdatePartNumber] 2,2,'072-4608-%%%%%%_(VV)'
-- =============================================
...
AS
BEGIN
	IF NOT EXISTS ( SELECT 1 FROM tblConfigurationDefinitionPartNumber WHERE ConfigurationDefinitionID = @ConfigurationDefinitionID  AND PartNumberID = @PartNumberID )
		BEGIN
			INSERT INTO tblConfigurationDefinitionPartNumber (ConfigurationDefinitionID, PartNumberID,Value) VALUES (@ConfigurationDefinitionID,@PartNumberID, @Value)
		END
	ELSE 
		BEGIN
			UPDATE tblConfigurationDefinitionPartNumber SET Value = @Value WHERE  ConfigurationDefinitionID = @ConfigurationDefinitionID and  PartNumberID= @PartNumberID 
		END
...
From ( Select Src=1, ID, B.*
         ...
         Cross Apply (Select [Key], Value From OpenJson((Select A.ID,A.FourLetID,A.ThreeLetID,A.Lat,A.Lon,A.City For JSON Path,Without_Array_Wrapper,INCLUDE_NULL_VALUES))) 
               B
        Union All
        Select Src=2, ID, B.*
...
from (
    ...
    where
		cmap.ConfigurationID = @configurationId and cmap.IsDeleted=0 and
...
select 
	GeoRefId,
	Description,
	AsxiCatTypeId as GeoRefIdCatTypeId,
	RegionID,
	CountryID,
	Elevation,
	Population,
	MapStatsAppearance as LayerDisplay,
	isInteractiveSearch as ISearch,
	isRliPoi as RLIPOI,
	isInteractivePOI as IPOI,
	isWorldClockPOI as WCPOI,
	isMakkahPOI as MakkahPOI,
	isClosestPOI as ClosestPOI,
	Lat,
	Lon,
	CustomChangeBitMask as CustomChangeBit,
	en as Lang_EN,
	fr as Lang_FR,
	de as Lang_DE,
	es as Lang_ES,
...
from (
	select 
		...
		tblgeoref.AsxiCatTypeId,
		tblgeoref.regionid,
		tblgeoref.countryid,
		tblgeoref.MapStatsAppearance,
		tblgeoref.isInteractiveSearch,
		tblgeoref.isrlipoi,
		tblgeoref.isInteractivePoi,
		tblgeoref.isWorldClockPoi,
		tblgeoref.isClosestPoi,
		tblgeoref.ismakkahpoi,
		tblgeoref.customchangebitmask,
	...
	where tblgeorefmap.configurationid = @configurationId and
		tblgeoref.georefid not between 200172 and 200239 and
		tblgeoref.georefid not between 300000 and 307840 and
		...
		tblcoveragesegmentmap.configurationid = @configurationId and tblcoveragesegmentmap.IsDeleted=0 and
...
from (
    ...
    from tblRegionSpelling 
		...
		inner join tbllanguagesmap as lmap on lmap.languageid = tbllanguages.id
    where
		rsmap.ConfigurationID = @configurationId and rsmap.isDeleted=0 and
		lmap.ConfigurationID = @configurationId and lmap.isDeleted=0
) as sourcetable
pivot(
    max(RegionName)
...
BEGIN
           ...
           SELECT DISTINCT Nodes.item.value('(.)[1]', 'VARCHAR(MAX)') as info_params
            FROM cust.tblWebMain CROSS APPLY WebMainItems.nodes('/webmain/infoitems/infoitem') Nodes(item)
...
-- Sample: EXEC [dbo].[SP_GetLanguagesOverride] 18
-- =============================================
...
BEGIN
                       ...
                       SELECT Global.value('(/language_set)[1]', 'varchar(max)') AS lang_list,
                        ISNULL(Nodes.item.value('(./@default)[1]','varchar(max)'),'') AS default_lang 
                        FROM cust.tblGlobal b CROSS APPLY b.Global.nodes('/language_set') Nodes(item) 
...
-- Sample: EXEC [dbo].[SP_GetScriptItemTypes]
-- =============================================
...
BEGIN
       ...
       SELECT *  
             ...
             (SELECT dbo.tblFeatureSet.Value as Name  
               ...
               WHERE dbo.tblFeatureSet.Name = 'script') as NameTable,  
                (SELECT dbo.tblFeatureSet.Value as DisplayName  
               FROM dbo.tblFeatureSet  
               WHERE dbo.tblFeatureSet.Name = 'script-display') as DisplayNameTable
END
GO
...
BEGIN
	...
	BEGIN
		...
		ELSE IF EXISTS (SELECT 1 FROM tblTasks WHERE ConfigurationID = @configurationId AND TaskTypeID IN (SELECT ID FROM tblTaskType WHERE Name = 'UI Merge Configuration') AND TaskStatusID = 4 AND TaskDataJSON = CAST(@parentConfigurationId AS NVARCHAR))
		BEGIN
			SET @mergeDate = (SELECT TOP 1 DateStarted FROM tblTasks WHERE ConfigurationID = @configurationId AND TaskTypeID IN (SELECT ID FROM tblTaskType WHERE Name = 'UI Merge Configuration') AND TaskStatusID = 4
									AND TaskDataJSON = CAST(@parentConfigurationId AS NVARCHAR) ORDER BY DateLastUpdated DESC)
...
BEGIN
		...
		SET  @sql =' SELECT * FROM (dbo.AspNetUsers INNER JOIN dbo.UserRoleAssignments ON dbo.AspNetUsers.Id = dbo.UserRoleAssignments.UserID) 
...
-- Sample: [dbo].[SP_global_AddLanguagesCode] 'EN',67
-- =============================================
...
BEGIN
                ...
                SELECT  LOWER(dbo.tblLanguages.Name) FROM dbo.tblLanguages 
...
-- Sample: EXEC [dbo].[SP_global_GetAllLanguages] 1
-- =============================================
...
BEGIN
                    SELECT L.ID, L.LanguageID, L.Name, L.NativeName, L.Description, ISNULL(L.ISLatinScript, 0) AS ISLatinScript, L.Tier, L.[2LetterID_4xxx],
					L.[3LetterID_4xxx], L.[2LetterID_ASXi], L.[3LetterID_ASXi], L.HorizontalOrder, L.HorizontalScroll, L.VerticalOrder, L.VerticalScroll
                    FROM tblLanguages L INNER JOIN tblLanguagesMap LM ON L.ID = LM.LanguageID 
                    INNER JOIN cust.tblGlobalMap ON cust.tblGlobalMap.ConfigurationID = LM.ConfigurationID 
                    WHERE LM.ConfigurationID = @configurationId
		...
		BEGIN TRY
			BEGIN TRANSACTION
				...
				BEGIN
					...
					ColEnd = @ColEnd, LatStart = @LatStart, LatEnd = @LatEnd, LongStart = @LongStart, LongEnd = @LongEnd, IsHf = @IsHf, Cdata = @Cdata
...
-- Sample EXEC [dbo].[SP_Makkah_GetMakkahPrayerTimes] 67
-- =============================================
...
BEGIN
	...
	DECLARE @viewsMenu NVARCHAR(500)
	SET @viewsMenu = (SELECT Value FROM tblFeatureSet WHERE Name = 'viewsmenu' AND FeatureSetID = 1)
	...
	SELECT distinct name, preset FROM @tblViewsMenu
END
GO
GO
	...
	ELSE IF (@type = 'update')
		BEGIN
			...
			begin
				...
				exec dbo.SP_ConfigManagement_HandleUpdate @configurationId, 'tblMenu', @mappedMenuId, @updateKey out
				UPDATE M
				SET perspective = @xmlValue FROM  cust.config_tblMenu(@configurationId) as M WHERE M.MenuID = @updateKey
