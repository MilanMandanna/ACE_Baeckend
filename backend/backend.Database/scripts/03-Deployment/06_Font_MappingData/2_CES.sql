...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @ConfigID, ID, 0, 0, 'adding' FROM tblFont WHERE ID BETWEEN 1 AND 935
		END
		--tblFontCategory
		...
		BEGIN
			...
			SELECT @ConfigID, FontCategoryID, 0, 0, 'adding' FROM tblFontCategory WHERE FontCategoryID BETWEEN 1 AND 4046
		END
		...
		BEGIN
			...
			SELECT @ConfigID, FontFamilyID, 0, 0, 'adding' FROM tblFontFamily WHERE FontFamilyID BETWEEN 1 AND 24
		END
		--tblFontMarker
