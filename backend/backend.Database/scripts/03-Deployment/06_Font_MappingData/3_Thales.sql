...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @ConfigID, ID, 0, 0, 'adding' FROM tblFont WHERE ID BETWEEN 3017 AND 3924
		END
		--tblFontCategory
		...
		BEGIN
			...
			SELECT @ConfigID, FontFamilyID, 0, 0, 'adding' FROM tblFontFamily WHERE FontFamilyID BETWEEN 26 AND 49
		END
		--tblFontMarker
